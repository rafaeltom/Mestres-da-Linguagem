                ...
                {data.schools.map((school: School) => (
                    <div key={school.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                        <div className="bg-slate-50 p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-100 gap-4">
                            ...
                            <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                                <Button variant="icon" onClick={(e: any) => { e.stopPropagation(); openModal('school', 'edit', school); }} title="Editar Escola"><i className="fas fa-calendar-alt"></i></Button>
                                ...
                                <div key={cls.id} className="border border-slate-200 p-4 rounded-xl hover:border-indigo-300 transition-colors bg-white relative group">
                                    <div className="flex flex-col items-start mb-2 pr-16 gap-1 w-full">
                                        ...
                                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                                            <button onClick={() => { setSelectedSchoolId(school.id); setSelectedClassId(cls.id); setShowEditStudents(true); }} className="bg-slate-100 text-slate-500 text-[10px] font-bold px-2 py-1 rounded-full flex-shrink-0 hover:bg-indigo-100 hover:text-indigo-600 transition-colors cursor-pointer" title="Editar alunos da turma">{cls.students?.length || 0} alunos</button>
                                            <div
                                                ...
                                                title="Clique ou passe o mouse para revelar o código"
                                            >
                                    ...
                                    <div className="flex gap-2 mt-4">
                                        <button onClick={() => { setSelectedSchoolId(school.id); setSelectedClassId(cls.id); setShowBatchImport(true); }} className="flex-1 bg-emerald-50 text-emerald-600 text-[10px] sm:text-xs font-bold py-2 rounded-lg hover:bg-emerald-100 border border-emerald-100 flex items-center justify-center gap-1 transition-colors">
                                            <i className="fas fa-file-import"></i> Importar
                                        </button>
                                        <button onClick={() => { setSelectedSchoolId(school.id); setSelectedClassId(cls.id); setShowEditStudents(true); }} className="flex-1 bg-indigo-50 text-indigo-600 text-[10px] sm:text-xs font-bold py-2 rounded-lg hover:bg-indigo-100 border border-indigo-100 flex items-center justify-center gap-1 transition-colors">
                                            <i className="fas fa-users-cog"></i> Editar alunos
                                        </button>
                                    </div>
                                    <div className="absolute top-3 right-3 flex gap-1 bg-white p-1 rounded-lg border border-slate-100">
                                        <button onClick={(e: any) => { e.stopPropagation(); openModal('class', 'edit', cls); }} className="p-1 text-slate-400 hover:text-indigo-500 w-7 h-7 flex items-center justify-center" title="Editar Dados Base da Turma"><i className="fas fa-pen text-xs"></i></button>
